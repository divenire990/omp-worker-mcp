...
3. **Preserve existing entries**: Ensure `"mcp__omp_worker"` is included alongside any existing direct namespaces (such as `"mcp__fastctx"`). Do not overwrite or drop your existing entries:
   ```toml
   # Preserve existing entries and include mcp__omp_worker
...
- **Natural Language Requests**: Simply describe your goal (e.g., *"Implement exponential backoff retry in the runner and verify unit tests pass"* or *"Refactor the auth middleware and user service in parallel"*). The host will autonomously consult the policy and delegate to `omp_run_compact` or `omp_run_batch_compact`.
- **Explicit Mention**: Explicitly asking for `omp-worker-mcp` is only necessary during initial verification or when the host unexpectedly falls back to direct execution for a large, multi-step task.
