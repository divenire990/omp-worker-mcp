...
| [**ChatGPT Secure MCP Tunnel 接入指南**](chatgpt-secure-tunnel.zh-CN.md) | 将网页端 ChatGPT 连接到本机 `omp-worker-mcp` 的远程 Supervisor-Worker 方案。 | • OpenAI `tunnel-client` stdio MCP 接入<br>• `OMP_WORKER_ALLOWED_ROOTS` 目录白名单<br>• symlink / junction 逃逸防护<br>• runtime 健康检查与信任边界 |
...
- **要从网页 ChatGPT 调用本机 OMP？** 请阅读 [ChatGPT Secure MCP Tunnel 接入指南](chatgpt-secure-tunnel.zh-CN.md)。
